...
-- tb_test2 테이블: tb_test1 테이블의 col1 컬럼하나당 10개의 레코드를 가지며 총 1000건이 있다.
...
-- 먼저 tb_test1 이 col2에 대해서 인덱스가 있는경우를 생각해보자.
-- 그러면 인덱스를 사용한 정렬의 조건을 부합하게 된다.
-- 왜냐하면 첫번째 테이블에 속해있기도 하고, 인덱스 순서도 맞고, where 조건에 사용된 컬럼이 PK라면 orderby 의 세컨더리 인덱스와 결국 같다.
